...
export default function CoachDashboard() {
  ...
  const AI_ACTIONS = [
    ...
    { id:"weekPlan",     icon:"📅", label:"Weekly Training Plan",       desc:"Priority plan for the full week" },
    { id:"revenueGoal",  icon:"💵", label:"$800 Goal Check",            desc:"Status toward weekly revenue goal" },
    { id:"urgentPlan",   icon:"🔥", label:`Handle ${urgent.length} Urgent Athletes`, desc:"Who to call first and what to say" },
    { id:"inactiveReach",icon:"🔁", label:`Re-Engage ${inactive.length} Inactive`,  desc:"Strategy to bring them back" },
    { id:"growthMove",   icon:"📈", label:"Top Growth Move",            desc:"Biggest opportunity this week" },
  ];

  async function generateAI(action: string) {
    setLoading(true); setResult(""); setError(""); setAiAction(action);
    const rosterSummary = athletes.map(a => `${a.name}(${a.sport},${a.sessions}sess,${a.status})`).join(" | ");
    ...
    const prompts: Record<string,string> = {
      dailyBrief: `COO AI for Coach T at Elite Skillz Lab 🧪, D1 Training Hulen Fort Worth. Today: ${dayName}. ${isD1?"D1 LOCKED 5:45-7:45 PM.":""}\nRoster: ${rosterSummary}\nUrgent: ${urgentList||"none"}\nTop 3 actions today, who needs immediate attention, #1 message to send right now. Direct, specific, no fluff.`,
      weekPlan: `Coach T — Elite Skillz Lab 🧪. D1 locked Tue/Wed 5:45-7:45PM. $800/week goal.\nRoster: ${rosterSummary}\nComplete weekly priority plan. Each day Mon-Fri: athlete focus, training themes, business actions. Specific and actionable.`,
      revenueGoal: `Business brain — Elite Skillz Lab 🧪. Coach T earns $25/hr + 10% commission. Goal: $800/week.\n${athletes.length} athletes. Urgent renewals: ${urgentList||"none"}.\nHow close to $800/week? Top 3 actions to close the gap? Who to call first? Direct with numbers.`,
      urgentPlan: `Coach T has ${urgent.length} urgent renewals: ${urgentList||"none"}. Each = $25+/hr. Specific action plan: who first, what to say, in what order. Write the exact text for the #1 priority renewal.`,
      inactiveReach: `Coach T has ${inactive.length} inactive athletes: ${inactive.map(a=>`${a.name}(${a.notes||"no notes"})`).join(", ")||"none"}. Who is most likely to return? Outreach order? Write the first message to send to the highest-probability one.`,
      growthMove: `Elite Skillz Lab 🧪 — ${athletes.length} athletes, ${urgent.length} urgent, $800/week goal.\nSingle highest-impact growth move THIS WEEK. Consider: adding athletes, increasing frequency, D1 promos, referrals, content. ONE specific action with exact steps. Not 5 things — the ONE best move.`,
    };
        ...
        {mounted && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8, marginTop:16 }}>
            {[
              ...
              {l:"INACTIVE",v:inactive.length,c:"rgba(255,255,255,0.4)"},
              {l:"ACTIVE",v:athletes.filter(a=>a.status==="active").length,c:"#00d084"},
            ].map((s,i)=>(
      ...
      <div style={{ padding:"16px" }}>
        ...
        <div style={{ background:"rgba(26,110,255,0.04)", border:"1px solid rgba(26,110,255,0.2)", borderRadius:16, padding:"16px", marginBottom:14 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            ...
            {(result||loading) && (
              <button onClick={()=>{setResult("");setError("");setAiAction(null);}} style={{ background:"transparent", border:"1px solid rgba(255,255,255,0.1)", color:"rgba(255,255,255,0.4)", borderRadius:8, padding:"4px 10px", cursor:"pointer", fontSize:11 }}>← Back</button>
            )}
          </div>
            ...
            {urgent.map((a:any) => (
              <Link key={a.id} href="/coach/agent" style={{ textDecoration:"none" }}>
                <div style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid rgba(255,68,68,0.08)" }}>
                  ...
                  <div style={{ fontSize:11, color:"#ff4444" }}>{a.sessions} left → ⚡ AI</div>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div style={{ marginTop:14, background:"rgba(26,110,255,0.04)", border:"1px solid rgba(26,110,255,0.15)", borderRadius:14, padding:"14px" }}>
          <div style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", color:"#1a6eff", marginBottom:10 }}>🔒 LOCKED COMMITMENTS</div>
