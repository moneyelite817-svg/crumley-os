...
export default function SchedulePage() {
  ...
  const daySessions = sessions.filter(s=>s.day===selDay).sort((a:any,b:any)=>a.time.localeCompare(b.time));
  const today = new Date().getDay();
  const todaySessions = sessions.filter(s=>s.day===today);
  const isLocked = (s:any)=>s.notes?.includes("LOCKED");

  const AI_ACTIONS = [
    {id:"dayBrief",icon:"📋",label:"Today's Session Brief",desc:"Complete plan for all sessions today"},
    {id:"announcement",icon:"📢",label:"Class Announcement",desc:"Text or post to send to athletes"},
    {id:"groupPlan",icon:"🏋️",label:"Group Class Plan",desc:"60-min group workout for D1 class"},
    {id:"schedule",icon:"📅",label:"Weekly Overview",desc:"Summary of the week's training priorities"},
  ];

  async function generateAI(action: string) {
    setAiLoading(true); setAiResult(""); setAiError("");
    const dayName = FDAYS[selDay];
    const daySess = sessions.filter(s=>s.day===selDay);
    const athletes = daySess.flatMap((s:any)=>s.clients||[]).join(", ");
    const prompts: Record<string,string> = {
      dayBrief: `You are Coach T at Elite Skillz Lab 🧪, D1 Training Hulen Fort Worth. Today is ${dayName}. Sessions: ${daySess.map((s:any)=>`${s.type} ${s.time}–${s.end}${s.clients?.length?` (${s.clients.join(", ")})`:" (group)"}`).join(" | ")||"No sessions scheduled"}. Write a complete coaching day brief: what to focus on for each session, energy management across the day, any key athletes to watch. Sound like a prepared, professional head coach. Under 8 sentences.`,
      announcement: `You are Coach T at Elite Skillz Lab 🧪. It's ${dayName}. Write a motivational announcement text/post for today's ${daySess.filter((s:any)=>!isLocked(s)).length} session(s). Hype the athletes, set the tone, remind them what they're training for. Short, powerful, coach energy. Under 4 sentences.`,
      groupPlan: `You are Coach T at D1 Training Hulen Fort Worth. Write a complete 60-minute group class plan for ${dayName}. Dumbbells and bands ONLY. Mixed ages and athletic levels. High energy, competitive atmosphere. Format: Activation (5min), Power (12min), Strength (25min), Conditioning (12min), Cooldown (6min). Include sets, reps, and coaching cues.`,
      schedule: `You are Coach T at Elite Skillz Lab 🧪. Summarize this week's training priorities: D1 locked blocks Tue/Wed 5:45-7:45 PM, ${sessions.filter((s:any)=>!isLocked(s)).length} private/personal sessions total. Write a short coaching overview for the week — what's the development focus, who needs attention, what's the revenue opportunity. Under 6 sentences. COO energy.`,
    };
    try {
      ...
      if(data.error) setAiError(data.message||data.error); else setAiResult(data.text);
      ...
      <main style={{minHeight:"100vh",background:"#000",paddingBottom:60,fontFamily:"system-ui"}}>
        ...
        <div style={{padding:"20px 16px"}}>
          <div style={{background:`${color}18`,border:`1px solid ${color}44`,borderRadius:14,padding:"18px",marginBottom:14}}>
            <div style={{fontSize:22,fontWeight:900,color,marginBottom:4}}>{sess.type}{isLocked(sess)?" 🔒":""}</div>
            <div style={{fontSize:13,color:"rgba(255,255,255,0.5)"}}>{sess.time}–{sess.end} · {FDAYS[sess.day]}</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}>
              {[{l:"BOOKED",v:`${sess.clients?.length||0}/${sess.maxClients}`},{l:"TYPE",v:isLocked(sess)?"LOCKED":"OPEN"}].map((s,i)=>(
          ...
          {sess.clients?.length>0&&(
            <div style={{background:"#111",border:"1px solid #222",borderRadius:12,padding:"16px",marginBottom:14}}>
              <div style={{fontSize:10,letterSpacing:"0.12em",color:"rgba(255,255,255,0.3)",fontWeight:700,marginBottom:10}}>BOOKED CLIENTS</div>
          ...
          <Link href="/coach/agent" style={{display:"block",textDecoration:"none",background:"#1a6eff",borderRadius:12,padding:"14px",textAlign:"center",fontSize:14,fontWeight:700,color:"#fff"}}>
            ⚡ Generate AI Workout Plan for This Session
          </Link>
        </div>
      </main>
    );
  ...
  return(
    <main style={{minHeight:"100vh",background:"#000",paddingBottom:80,fontFamily:"system-ui"}}>
      <div style={{background:"rgba(0,0,0,0.95)",backdropFilter:"blur(20px)",borderBottom:"1px solid #222",padding:"14px 16px",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>setShowAI(!showAI)} style={{background:"rgba(26,110,255,0.15)",border:"1px solid rgba(26,110,255,0.3)",color:"#1a6eff",borderRadius:10,padding:"8px 12px",cursor:"pointer",fontSize:12,fontWeight:700}}>⚡ AI</button>
        ...
        <div style={{display:"flex",gap:4}}>
          {DAYS.map((d,i)=>{
            ...
            const isToday=i===today; const active=i===selDay;
            return(
      ...
      {/* AI Panel */}
      {showAI&&(
        <div style={{background:"rgba(26,110,255,0.04)",border:"1px solid rgba(26,110,255,0.2)",borderRadius:16,margin:"14px 16px 0",padding:"16px"}}>
          <div style={{fontSize:13,fontWeight:900,color:"#1a6eff",marginBottom:12}}>⚡ AI COACHING TOOLS</div>
          {!aiResult&&!aiLoading&&(
            <div>
              {AI_ACTIONS.map(a=>(
                <div key={a.id} onClick={()=>{setAiAction(a.id);generateAI(a.id);}} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"12px 14px",marginBottom:8,cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
          ...
          {aiLoading&&<div style={{textAlign:"center",padding:"20px 0"}}><div style={{fontSize:28}}>⚙️</div><div style={{fontSize:13,fontWeight:800,color:"#1a6eff",marginTop:8,letterSpacing:"0.1em"}}>GENERATING...</div></div>}
          {aiError&&<div style={{background:"rgba(255,68,68,0.08)",border:"1px solid rgba(255,68,68,0.2)",borderRadius:10,padding:"12px",color:"#ff8888",fontSize:13}}>{aiError}</div>}
          {aiResult&&(
            <div>
              <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"14px",fontSize:14,lineHeight:1.8,color:"#fff",whiteSpace:"pre-wrap",marginBottom:12}}>{aiResult}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <button onClick={()=>{setAiResult("");setAiError("");setAiAction(null);}} style={{background:"rgba(255,255,255,0.05)",border:"1px solid #333",color:"rgba(255,255,255,0.4)",borderRadius:10,padding:"12px",cursor:"pointer",fontSize:12}}>← More</button>
      ...
      <div style={{padding:"14px 16px"}}>
        ...
        {daySessions.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:"rgba(255,255,255,0.3)",fontSize:14}}>No sessions · tap + BOOK to add</div>}
        {daySessions.map((sess:any)=>{
          const color=COLORS[sess.type]||"#1a6eff"; const locked=isLocked(sess);
          return(
            <div key={sess.id} onClick={()=>setSelSession(sess)} style={{background:"#111",border:"1px solid #222",borderLeft:`4px solid ${locked?"#1a6eff":color}`,borderRadius:12,padding:"14px",marginBottom:10,cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{fontSize:14,fontWeight:700,color:"rgba(255,255,255,0.5)"}}>{sess.clients?.length||0}/{sess.maxClients}</div>
              </div>
      ...
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:100,display:"flex",alignItems:"flex-end"}}>
          <div style={{background:"#111",borderRadius:"16px 16px 0 0",padding:"24px 16px 48px",width:"100%",maxHeight:"80vh",overflowY:"auto"}}>
            ...
            <select value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value}))} style={inp}>{Object.keys(COLORS).map(t=><option key={t}>{t}</option>)}</select>
            <label style={lbl}>DAY</label>
            <select value={form.day} onChange={e=>setForm(p=>({...p,day:parseInt(e.target.value)}))} style={inp}>{FDAYS.map((d,i)=><option key={i} value={i}>{d}</option>)}</select>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
