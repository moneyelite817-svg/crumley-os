...
interface InventoryItem {
  ...
  damageNotes: string; repairNeeded: boolean; soldDisposed: boolean;
  notes: string; createdAt: string;
}

const CATEGORIES = [
  {id:"sofa",      label:"Sofas",            icon:"🛋"},
  {id:"bed",       label:"Beds & Bedding",    icon:"🛏"},
  {id:"table",     label:"Tables",            icon:"🪑"},
  {id:"chair",     label:"Chairs",            icon:"🪑"},
  {id:"rug",       label:"Rugs",              icon:"🟫"},
  {id:"artwork",   label:"Artwork & Mirrors", icon:"🖼"},
  {id:"lamp",      label:"Lamps & Lighting",  icon:"💡"},
  {id:"decor",     label:"Décor & Accessories",icon:"🏺"},
  {id:"staging",   label:"Staging Kits",      icon:"📦"},
  {id:"moving",    label:"Moving Equipment",  icon:"🚛"},
  {id:"other",     label:"Other",             icon:"📋"},
];

const CONDITIONS: {id:InventoryItem["condition"];label:string;color:string;bg:string}[] = [
...
const JOBS_STORAGE = "cros_luxury_jobs_v3";

const BLANK: Omit<InventoryItem,"id"|"createdAt"> = {
  name:"", category:"sofa", quantity:1, condition:"good",
  purchasePrice:0, currentValue:0, purchaseDate:"",
  vendor:"", currentLocation:"Storage", assignedJob:"",
  damageNotes:"", repairNeeded:false, soldDisposed:false, notes:"",
};

const inp:any={width:"100%",padding:"10px 12px",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,color:"#fff",fontFamily:"system-ui",fontSize:14,outline:"none",boxSizing:"border-box",marginBottom:10};
...
export default function InventoryPage() {
  ...
  useEffect(()=>{
    ...
    try{const s=localStorage.getItem(JOBS_STORAGE);if(s)setJobs(JSON.parse(s));}catch{}
  },[]);
  ...
  function save(){
    ...
    if(view==="edit"&&sel){
      persist(items.map(i=>i.id===sel.id?{...i,...form}:i));
      setSel({...sel,...form}); setView("detail");
    } else {
      persist([...items,{...form,id:`inv-${Date.now()}`,createdAt:new Date().toISOString()}]);
  ...
  function quickAction(id:string, action:string){
    const updates: Partial<InventoryItem> = {
      ...
      repaired:{condition:"good",repairNeeded:false,damageNotes:""},
      sold:{condition:"sold",soldDisposed:true},
    }[action]||{};
    persist(items.map(i=>i.id===id?{...i,...updates}:i));
    if(sel?.id===id)setSel(p=>p?{...p,...updates}:p);
  }
  ...
  const totalPurchase = activeItems.reduce((a,i)=>a+(i.purchasePrice||0)*i.quantity,0);
  ...
  const assigned = activeItems.filter(i=>i.assignedJob).length;
    ...
    <main style={{minHeight:"100vh",background:"#03060f",paddingBottom:80,fontFamily:"system-ui"}}>
      ...
      <div style={{padding:"20px 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
          {CATEGORIES.map(cat=>(
            <div key={cat.id} onClick={()=>f("category",cat.id)} style={{background:form.category===cat.id?"rgba(26,110,255,0.2)":"rgba(255,255,255,0.03)",border:`1px solid ${form.category===cat.id?"rgba(26,110,255,0.4)":"rgba(255,255,255,0.06)"}`,borderRadius:10,padding:"8px",cursor:"pointer",textAlign:"center" as const}}>
              <div style={{fontSize:18}}>{cat.icon}</div>
              <div style={{fontSize:10,color:form.category===cat.id?"#1a6eff":"rgba(255,255,255,0.5)",marginTop:3,fontWeight:form.category===cat.id?700:400}}>{cat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap" as const,marginBottom:12}}>
          {CONDITIONS.map(c=>(
            <button key={c.id} onClick={()=>f("condition",c.id)} style={{padding:"7px 14px",borderRadius:100,background:form.condition===c.id?c.bg:"rgba(255,255,255,0.04)",border:`1px solid ${form.condition===c.id?c.color:"rgba(255,255,255,0.08)"}`,color:form.condition===c.id?c.color:"rgba(255,255,255,0.4)",fontSize:11,fontWeight:700,cursor:"pointer"}}>
              {c.label}
            </button>
          ))}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div><label style={lbl}>PURCHASE PRICE $</label><input type="number" value={form.purchasePrice} onChange={e=>f("purchasePrice",parseFloat(e.target.value)||0)} style={inp}/></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        {[{l:"VENDOR / SOURCE",k:"vendor",p:"Where purchased"},{l:"CURRENT LOCATION",k:"currentLocation",p:"Storage, 3031 Valentine, etc."},{l:"NOTES",k:"notes",p:"Any additional notes"}].map(field=>(
        ...
        {form.condition==="damaged"&&(
          <div>
            ...
            <textarea value={form.damageNotes} onChange={e=>f("damageNotes",e.target.value)} placeholder="Describe the damage..." style={{...inp,height:60,resize:"none" as const}}/>
  ...
  if(view==="detail"&&sel){
    ...
    const assignedJobName=sel.assignedJob?jobs.find(j=>j.id===sel.assignedJob)?.address?.split(",")[0]||"Job":"Unassigned";
    return(
      <main style={{minHeight:"100vh",background:"#03060f",paddingBottom:60,fontFamily:"system-ui"}}>
        ...
        <div style={{padding:"20px 16px"}}>
          <div style={{background:cond.bg,borderLeft:`4px solid ${cond.color}`,border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"18px",marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
            {[{l:"PURCHASE $",v:`$${(sel.purchasePrice||0).toLocaleString()}`,c:"rgba(255,255,255,0.7)"},{l:"CURRENT VALUE",v:`$${(sel.currentValue||sel.purchasePrice||0).toLocaleString()}`,c:"#1a6eff"},{l:"QTY",v:sel.quantity,c:"#fff"}].map((s,i)=>(
          ...
          {[{l:"LOCATION",v:sel.currentLocation||"—"},{l:"VENDOR",v:sel.vendor||"—"},{l:"PURCHASED",v:sel.purchaseDate||"—"},{l:"NOTES",v:sel.notes||"—"}].map((r,i)=>(
            <div key={i} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"12px 14px",marginBottom:8}}>
              <div style={{...lbl,marginBottom:3}}>{r.l}</div>
              <div style={{fontSize:14,color:r.v==="—"?"rgba(255,255,255,0.25)":"#fff"}}>{r.v}</div>
            </div>
          ))}
          {sel.damageNotes&&(
            <div style={{background:"rgba(255,68,68,0.06)",border:"1px solid rgba(255,68,68,0.2)",borderRadius:12,padding:"14px",marginBottom:8}}>
              <div style={{...lbl,marginBottom:4,color:"rgba(255,68,68,0.7)"}}>DAMAGE NOTES</div>
    ...
    <main style={{minHeight:"100vh",background:"#03060f",paddingBottom:80,fontFamily:"system-ui"}}>
      ...
      <div style={{padding:"16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          {[
            {l:"TOTAL INVENTORY VALUE",v:`$${totalValue.toLocaleString()}`,s:`${activeItems.length} items`,c:"#1a6eff",big:true},
            {l:"TOTAL PAID",v:`$${totalPurchase.toLocaleString()}`,s:`${totalValue>totalPurchase?"+" :""}$${(totalValue-totalPurchase).toLocaleString()} ${totalValue>=totalPurchase?"gain":"loss"}`,c:"#00d084",big:true},
            {l:"DAMAGED",v:damaged,s:`${damaged} item${damaged!==1?"s":""} need attention`,c:"#ff4444",big:false},
            {l:"ASSIGNED TO JOBS",v:assigned,s:`active staging`,c:"#f0c040",big:false},
          ].map((s,i)=>(
            <div key={i} style={{background:`${s.c}12`,border:`1px solid ${s.c}33`,borderRadius:14,padding:"16px"}}>
              <div style={{fontSize:s.big?24:28,fontWeight:900,color:s.c,marginBottom:4}}>{s.v}</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:"0.1em",textTransform:"uppercase" as const,marginBottom:2}}>{s.l}</div>
        ...
        {/* By Category */}
        <div style={{fontSize:11,fontWeight:700,letterSpacing:"0.12em",color:"rgba(255,255,255,0.35)",textTransform:"uppercase" as const,marginBottom:12}}>BY CATEGORY</div>
          ...
          {CATEGORIES.map(cat=>{
            ...
            return(
              <div key={cat.id} onClick={()=>{setCatFilter(cat.id);setView("list");}} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"12px",cursor:"pointer",textAlign:"center"}}>
                ...
                <div style={{fontSize:10,color:"rgba(255,255,255,0.25)",marginTop:2}}>${catValue.toLocaleString()}</div>
              </div>
            );
          }).filter(Boolean)}
        </div>
        ...
        {(damaged>0||missing>0)&&(
          <div style={{background:"rgba(255,68,68,0.06)",border:"1px solid rgba(255,68,68,0.2)",borderRadius:14,padding:"14px",marginBottom:14}}>
            ...
            {items.filter(i=>i.condition==="damaged"||i.condition==="missing").slice(0,5).map(i=>{
              ...
              return(
                <div key={i.id} onClick={()=>{setSel(i);setView("detail");}} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid rgba(255,68,68,0.08)",cursor:"pointer"}}>
                  ...
                  <span style={{fontSize:10,padding:"3px 8px",background:cond.bg,color:cond.color,borderRadius:4,fontWeight:700,height:"fit-content",alignSelf:"center"}}>{cond.label}</span>
                </div>
              );
            })}
          </div>
    ...
    <main style={{minHeight:"100vh",background:"#03060f",paddingBottom:80,fontFamily:"system-ui"}}>
      <div style={{background:"rgba(3,6,15,0.95)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"14px 16px",position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",marginBottom:6}}>
          <button onClick={()=>setCatFilter("all")} style={{padding:"5px 12px",borderRadius:100,background:catFilter==="all"?"#1a6eff":"rgba(255,255,255,0.04)",border:`1px solid ${catFilter==="all"?"#1a6eff":"rgba(255,255,255,0.08)"}`,color:catFilter==="all"?"#fff":"rgba(255,255,255,0.4)",fontSize:10,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap" as const}}>ALL</button>
          {CATEGORIES.map(c=>activeItems.some(i=>i.category===c.id)&&(
            <button key={c.id} onClick={()=>setCatFilter(c.id)} style={{padding:"5px 12px",borderRadius:100,background:catFilter===c.id?"#1a6eff":"rgba(255,255,255,0.04)",border:`1px solid ${catFilter===c.id?"#1a6eff":"rgba(255,255,255,0.08)"}`,color:catFilter===c.id?"#fff":"rgba(255,255,255,0.4)",fontSize:10,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap" as const}}>{c.icon} {c.label}</button>
          ))}
        </div>
        <div style={{display:"flex",gap:6,overflowX:"auto"}}>
          {["all",...CONDITIONS.map(c=>c.id)].map(c=>(
            <button key={c} onClick={()=>setCondFilter(c)} style={{padding:"5px 12px",borderRadius:100,background:condFilter===c?"rgba(26,110,255,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${condFilter===c?"rgba(26,110,255,0.4)":"rgba(255,255,255,0.08)"}`,color:condFilter===c?"#1a6eff":"rgba(255,255,255,0.4)",fontSize:10,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap" as const,textTransform:"capitalize" as const}}>{c==="all"?"ALL CONDITIONS":c}</button>
          ))}
        </div>
      </div>
      <div style={{padding:"14px 16px"}}>
        ...
        {shown.map(item=>{
          ...
          const jobName=item.assignedJob?jobs.find(j=>j.id===item.assignedJob)?.address?.split(",")[0]:"In Storage";
          return(
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>{cat.label} · {item.quantity > 1?`Qty ${item.quantity} · `:""}{jobName}</div>
                  </div>
                </div>
                <div style={{textAlign:"right" as const,flexShrink:0}}>
                  ...
                  <span style={{fontSize:9,padding:"2px 8px",background:cond.bg,color:cond.color,borderRadius:4,fontWeight:700,marginTop:4,display:"block"}}>{cond.label}</span>
                </div>
              </div>
              {item.damageNotes&&<div style={{fontSize:11,color:"#ff8888",fontStyle:"italic"}}>{item.damageNotes}</div>}
            </div>
          );
        })}
      </div>
